...
yay <- lm(DepthToWater_m ~ IntegerYear, 
                     data = window_data)
...
subset_lm_yr2 <- rolling_lm_yr(subset, window_size = 5)
...
for(position in seq(1, nrow(data), by = 5)) {
...
for(group in seq_along(depth_id)) {
  #print(depth_id[[group]])
  
  subset <- depth_texas %>% 
    filter(StnID %in% depth_id[[1]][group]) %>% 
    mutate(year = IntegerYear)
  
  
  subset_lm <- breakpoint_fun(subset, window_size = 5) 
  
  subset_df <- rbind(subset_df, subset_lm)
}

#unique(subset_df$StnID) == unique(depth_id$StnID)
```
